...
void mostrar_est(int cant,est *a);
void mostrar_int(int cant,int *a);
int *mas(int cant);
...
void mostrar_est(int cant,est *a){
        ...
        for(i = 0; i < cant; i++){
                printf("\nEst c: %d, mensaje: %s\n", (a+i)->c, (a+i)->mensaje);
                mostrar_int((a+i)->cant_arr,(a+i)->arr);
                printf("\n");
        }
...
void fn2(int cant, est **res){
        est *data = (est *)malloc(sizeof(est)*cant);
...
int main(int argc, char **argv){
        ...
        est *res = NULL;
        fn2(cant, &res);
	mostrar_int(cant,arr);
        int *arr2 = mas(cant);
        mostrar_est(cant,res);
        mostrar_int(cant,arr2);
